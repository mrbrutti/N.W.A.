...
func (app *application) handleAdminToolsJSON(writer http.ResponseWriter, request *http.Request) {
	...
	case http.MethodGet:
		...
		writeJSON(writer, http.StatusOK, map[string]any{"tools": tools})
	case http.MethodPost:
...
func (app *application) handleAdminToolJSON(writer http.ResponseWriter, request *http.Request) {
	...
	toolID := strings.Trim(strings.TrimPrefix(request.URL.Path, "/api/v1/admin/tools/"), "/")
...
func (app *application) handlePlatformHealthJSON(writer http.ResponseWriter, request *http.Request) {
	_, _, ok := app.requirePlatformUser(writer, request, true)
	if !ok {
		return
	}
	health, err := app.platform.store.healthSummary()
...
func (app *application) handleEngagementInventoryJSON(writer http.ResponseWriter, request *http.Request) {
	user, _, ok := app.requirePlatformUser(writer, request, false)
	if !ok {
		return
	}
	path := strings.TrimPrefix(request.URL.Path, "/api/v1/engagements/")
	path = strings.Trim(path, "/")
	parts := strings.Split(path, "/")
	if len(parts) < 2 {
		http.NotFound(writer, request)
	...
	switch resource {
	case "summary":
		stats, err := app.platform.store.engagementStats(engagement.ID)
		if err != nil {
			http.Error(writer, http.StatusText(http.StatusInternalServerError), http.StatusInternalServerError)
			return
		}
	...
	case "zones":
		items, err := app.platform.store.listEngagementZones(engagement.ID)
		if err != nil {
			http.Error(writer, http.StatusText(http.StatusInternalServerError), http.StatusInternalServerError)
			return
		}
	...
	case "sources":
		items, err := app.platform.store.listEngagementSources(engagement.ID, 500)
		if err != nil {
			http.Error(writer, http.StatusText(http.StatusInternalServerError), http.StatusInternalServerError)
			return
		}
		writeJSON(writer, http.StatusOK, items)
	case "runs":
		items, err := app.platform.store.listEngagementRuns(engagement.ID, 500)
		if err != nil {
			http.Error(writer, http.StatusText(http.StatusInternalServerError), http.StatusInternalServerError)
			return
		}
		writeJSON(writer, http.StatusOK, items)
	case "hosts":
		items, err := app.platform.store.listEngagementHosts(engagement.ID, strings.TrimSpace(request.URL.Query().Get("zone")), strings.TrimSpace(request.URL.Query().Get("query")), 500)
		if err != nil {
			http.Error(writer, http.StatusText(http.StatusInternalServerError), http.StatusInternalServerError)
			return
		}
		writeJSON(writer, http.StatusOK, items)
	case "ports":
		items, err := app.platform.store.listEngagementPorts(engagement.ID, strings.TrimSpace(request.URL.Query().Get("query")), 500)
		if err != nil {
			http.Error(writer, http.StatusText(http.StatusInternalServerError), http.StatusInternalServerError)
			return
		}
		writeJSON(writer, http.StatusOK, items)
	case "findings":
		items, err := app.platform.store.listEngagementFindings(engagement.ID, strings.TrimSpace(request.URL.Query().Get("query")), request.URL.Query().Get("severity"), 500)
		if err != nil {
			http.Error(writer, http.StatusText(http.StatusInternalServerError), http.StatusInternalServerError)
			return
		}
	...
	case "topology":
		...
		writeJSON(writer, http.StatusOK, workspace.currentSnapshot().topology)
	case "recommendations":
		workspace, _, err := app.center.loadWorkspaceByID(engagement.LegacyWorkspaceID)
		if err != nil {
			http.Error(writer, http.StatusText(http.StatusInternalServerError), http.StatusInternalServerError)
		...
		writeJSON(writer, http.StatusOK, map[string]any{
			...
			"runs":            hostRunViews(workspace.plugins.recentJobs(12)),
		})
	case "stream":
		app.handleEngagementSSE(writer, request, user, engagement)
	default:
		http.NotFound(writer, request)
	}
}

func (app *application) handleEngagementSSE(writer http.ResponseWriter, request *http.Request, user platformUserRecord, engagement platformEngagementRecord) {
	...
	for {
		...
		_, _ = writer.Write([]byte("event: stats\n"))
